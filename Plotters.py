...
class Plotters(object):
	...
	def plot_matrix(self, M, inter=1):
		fig, ax = plt.subplots()
	...
	def visualize_graph_signal(self,G, graph_signal,title='Graph signal', ax=None, node_labels=0):
		'''
		G: PyGSP graph
		...
		ax1 = nx.draw_networkx_nodes(net, pos, nodelist= degree.keys(), node_size=190, node_color=graph_signal, ax=ax, cmap=plt.cm.jet, vmin=valor_min,vmax=valor_max)
		plt.colorbar(ax1)
