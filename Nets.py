...
class Nets(object):
	...
	def load_enron(self):
		...
		for n in net.nodes():
			net.node[n]['anomaly'] = anomalies.iloc[int(n)][1]
	...
	def load_disney(self):
		...
		for n in net.nodes():
			cod = net.node[n]['label'] # take the label from node
			net.node[n]['anomaly'] = anomalies.loc[cod][1]
			net.node[n]['title'] = code_titles[code_titles['code']==cod]['title'].values[0]
	...
	def load_toy_example(self):
		...
		# Anomalies
		G.node[8]['income']  = 2600   # Medium in rich
		G.node[135]['income'] = 9650  # Rich in very-poor
		G.node[50]['income'] =  600   # very-poor in medium

		G.node[8]['anomaly']  = 1  
		G.node[135]['anomaly'] = 1
		G.node[50]['anomaly']  = 1  
	...
	def load_books(self):
		...
		for n in net.nodes():
			net.node[n]['anomaly'] = anomalies.loc[int(n)][1]
